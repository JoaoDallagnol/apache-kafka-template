...
public class KafkaConsumerConfig {
    ...
    public ConsumerFactory<String, OrderCreatedEvent> orderCreatedConsumerFactory() {
        ...
        valueDeserializer.addTrustedPackages("com.example.orderprocessing.payment.event");
    ...
    public DefaultErrorHandler defaultErrorHandler(KafkaTemplate<String, Object> kafkaTemplate) {
        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(
                kafkaTemplate,
                (record, exception) -> new TopicPartition(topicProperties.ordersCreated().dltName(), record.partition())
        );
